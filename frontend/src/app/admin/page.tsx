...
export default function AdminPage() {
    ...
    const fetchReportList = async (pageNum = 1) => {
        ...
        try {
            const response = await fetch(
                `${process.env.BACKEND_API}/api/fetch-reported-posts/?page=${pageNum}`,
                { 
                    method: "GET", 
                    credentials: "include",
                    headers: {
    ...
    const fetchBanList = async (pageNum = 1) => {
        ...
        setLoading(true);
        try {
            const response = await axios.get(`${process.env.BACKEND_API}/api/fetch-banned-posts/`, {
                params: {
                    user_id: profile.id,
                    page: pageNum
                },
    ...
    const handleUnban = async (post: Post) => {
        ...
        try {
            const response = await axios.post(`${process.env.BACKEND_API}/api/post/unban/`, {
                post_id: post.id,
            });
    ...
    const handleBan = async (post: Post) => {
        ...
        try {
            const response = await axios.post(`${process.env.BACKEND_API}/api/post/ban/`, {
                post_id: post.id,
                admin_id: profile.id,
            });
